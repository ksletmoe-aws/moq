...
/// Build a `moq_track_config` for the given codec/name with optional CMAF init.
fn track_config(name: &[u8], codec: &[u8], init: Option<&[u8]>) -> moq_track_config {
	moq_track_config {
		size: std::mem::size_of::<moq_track_config>() as u32,
		name: name.as_ptr() as *const c_char,
		name_len: name.len(),
		codec: codec.as_ptr() as *const c_char,
		codec_len: codec.len(),
		init_data: init.map(|d| d.as_ptr()).unwrap_or(std::ptr::null()),
		init_data_len: init.map(|d| d.len()).unwrap_or(0),
		width: std::ptr::null(),
		height: std::ptr::null(),
		framerate: std::ptr::null(),
		sample_rate: std::ptr::null(),
		channel_count: std::ptr::null(),
		bitrate: std::ptr::null(),
	}
}

#[test]
fn slim_broadcast_aligned_groups() {
	// Mode B: create broadcast, create track, start group, write, close.
	let broadcast = id(moq_publish_create());
	let _guard = Guard(Some(|| {
		moq_publish_close(broadcast);
	}));

	let init = b"fake-cmaf-init";
	let name = b"video0";
	let codec = b"avc1.640028";
	let cfg = track_config(name, codec, Some(init));
	let track = id(unsafe { moq_track_create(broadcast, &cfg) });

	assert_eq!(moq_broadcast_start_group(broadcast, 0), 0);

	let payload = b"fake-moof-mdat";
	assert_eq!(
		unsafe { moq_broadcast_write(broadcast, track, payload.as_ptr(), payload.len()) },
		0
	);

	// Starting a new group finishes the previous one and begins another.
	assert_eq!(moq_broadcast_start_group(broadcast, 1), 0);
	assert_eq!(
		unsafe { moq_broadcast_write(broadcast, track, payload.as_ptr(), payload.len()) },
		0
	);

	// track_close auto finishes whatever group is open.
	assert_eq!(moq_track_close(broadcast, track), 0);
	// Double close fails.
	assert!(moq_track_close(broadcast, track) < 0);
}

#[test]
fn slim_per_track_groups() {
	// Mode A: create track, open group, write, close group.
	let broadcast = id(moq_publish_create());
	let _guard = Guard(Some(|| {
		moq_publish_close(broadcast);
	}));
	...
	let mut cfg = track_config(name, codec, None);
	cfg.sample_rate = &sample_rate;
	cfg.channel_count = &channel_count;
	let track = id(unsafe { moq_track_create(broadcast, &cfg) });

	let group = id(moq_group_open(track, 0));
	let payload = b"opus frame";
	assert_eq!(unsafe { moq_group_write(group, payload.as_ptr(), payload.len()) }, 0);
	assert_eq!(moq_group_close(group), 0);
	// Double close on the group handle fails.
	assert!(moq_group_close(group) < 0);

	assert_eq!(moq_track_close(broadcast, track), 0);
}

#[test]
fn slim_broadcast_write_without_start_group_errors() {
	let broadcast = id(moq_publish_create());
	let _guard = Guard(Some(|| {
		moq_publish_close(broadcast);
	}));
	...
	let name = b"video0";
	let codec = b"avc1.640028";
	let cfg = track_config(name, codec, None);
	let track = id(unsafe { moq_track_create(broadcast, &cfg) });

	let payload = b"data";
	let ret = unsafe { moq_broadcast_write(broadcast, track, payload.as_ptr(), payload.len()) };
	// NoActiveGroup maps to -31 in error.rs.
	assert_eq!(
		ret, -31,
		"broadcast_write before start_group should fail with NoActiveGroup"
	);
...
#[test]
fn slim_track_close_finishes_open_group() {
	// track_close on a track with an open Mode B group should succeed and
	// implicitly finish the group.
	let broadcast = id(moq_publish_create());
	let _guard = Guard(Some(|| {
		moq_publish_close(broadcast);
	}));
	...
	// Close while group is still open. Implementation must finish the group.
	assert_eq!(moq_track_close(broadcast, track), 0);

	// Subsequent operations on the closed track fail.
	assert!(unsafe { moq_broadcast_write(broadcast, track, payload.as_ptr(), payload.len()) } < 0);
}

#[test]
fn slim_track_close_reaps_orphaned_mode_a_group() {
	// Opening a Mode A group then closing the parent track without first
	// closing the group must invalidate the group handle. Subsequent
	// group_write and group_close calls fail with GroupNotFound (-30).
	let broadcast = id(moq_publish_create());
	let _guard = Guard(Some(|| {
		moq_publish_close(broadcast);
	}));
	...
	// Close the parent track without closing the group first.
	assert_eq!(moq_track_close(broadcast, track), 0);

	// The orphaned group handle is now invalid.
	assert_eq!(
		unsafe { moq_group_write(group, payload.as_ptr(), payload.len()) },
		-30,
		"group_write on orphaned Mode A group should fail with GroupNotFound"
	);
	assert_eq!(
		moq_group_close(group),
		-30,
		"group_close on orphaned Mode A group should fail with GroupNotFound"
	);
}

#[test]
fn slim_track_close_only_reaps_own_mode_a_groups() {
	// Closing one track must not invalidate Mode A groups on a sibling
	// track within the same broadcast.
	let broadcast = id(moq_publish_create());
	let _guard = Guard(Some(|| {
		moq_publish_close(broadcast);
	}));
	...
	let mut cfg_a = track_config(b"audio-a", b"opus", None);
	cfg_a.sample_rate = &sample_rate;
	cfg_a.channel_count = &channel_count;
	let track_a = id(unsafe { moq_track_create(broadcast, &cfg_a) });

	let mut cfg_b = track_config(b"audio-b", b"opus", None);
	cfg_b.sample_rate = &sample_rate;
	cfg_b.channel_count = &channel_count;
	let track_b = id(unsafe { moq_track_create(broadcast, &cfg_b) });

	let group_a = id(moq_group_open(track_a, 0));
	let group_b = id(moq_group_open(track_b, 0));

	// Closing track_a must not affect group_b on track_b.
	assert_eq!(moq_track_close(broadcast, track_a), 0);
	assert_eq!(moq_group_close(group_a), -30, "group_a should be reaped with track_a");

	let payload = b"opus frame";
	assert_eq!(
		unsafe { moq_group_write(group_b, payload.as_ptr(), payload.len()) },
		0,
		"group_b on surviving track_b should still accept writes"
	);
	assert_eq!(moq_group_close(group_b), 0);

	assert_eq!(moq_track_close(broadcast, track_b), 0);
}

#[test]
fn slim_broadcast_close_reaps_orphaned_mode_a_groups() {
	// Closing the broadcast must reap any Mode A groups whose owning track
	// belongs to that broadcast, even if the caller never called track_close.
	let broadcast = id(moq_publish_create());

	let sample_rate: u32 = 48_000;
	let channel_count: u32 = 2;
	let mut cfg = track_config(b"audio0", b"opus", None);
	cfg.sample_rate = &sample_rate;
	cfg.channel_count = &channel_count;
	let track = id(unsafe { moq_track_create(broadcast, &cfg) });
	...
	// Close the broadcast directly. Tracks and Mode A groups should both
	// be reaped.
	assert_eq!(moq_publish_close(broadcast), 0);
	...
	assert_eq!(
		unsafe { moq_group_write(group, payload.as_ptr(), payload.len()) },
		-30,
		"group_write on Mode A group orphaned by broadcast close should fail"
	);
	assert_eq!(
		moq_group_close(group),
		-30,
		"group_close on Mode A group orphaned by broadcast close should fail"
	);
	// The track is also gone.
	assert!(moq_track_close(broadcast, track) < 0);
}
